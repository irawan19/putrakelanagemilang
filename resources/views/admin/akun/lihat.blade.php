    ...
    <div class="row g-4 mb-4">
        <div class="col-md-12">
            <div class="card">
				...
				<form class="form-horizontal m-t-40" action="{{ URL('dashboard/akun/prosesedit')}}" method="POST">
					{{ csrf_field() }}
                    @method('PATCH')
                    <div class="card-body">
                        <div class="mb-3">
                            <label class="form-label" for="name">Nama <b style="color:red">*</b></label>
                            <input class="form-control {{ \App\Helpers\General::validForm($errors->first('name')) }}" id="name" type="text" name="name" value="{{Request::old('name') == '' ? Auth::user()->name : Request::old('name')}}">
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control {{ \App\Helpers\General::validForm($errors->first('username')) }}" id="username" type="text" name="username" value="{{Request::old('username') == '' ? Auth::user()->username : Request::old('username')}}">
                            {{\App\Helpers\General::pesanErrorForm($errors->first('username'))}}
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="email">Email <b style="color:red">*</b></label>
                            <input class="form-control {{ \App\Helpers\General::validForm($errors->first('email')) }}" id="email" type="email" name="email" value="{{Request::old('email') == '' ? Auth::user()->email : Request::old('email')}}">
