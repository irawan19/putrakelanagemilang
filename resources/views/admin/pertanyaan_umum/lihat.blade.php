                ...
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-md-6 right-align">
                            {{\App\Helpers\General::tambah('dashboard/pertanyaan-umum/tambah')}}
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <form method="GET" action="{{ URL('dashboard/pertanyaan-umum/cari') }}">
						@csrf
                        ...
                        <table id="tablesort" class="table table-responsive-sm table-bordered table-striped table-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="nowrap">Pertanyaan</th>
                                    <th class="nowrap">Jawaban</th>
                                </tr>
                            </thead>
                            <tbody>
                                @if(!$pertanyaan_umums->isEmpty())
					    			@php($no = 1)
		            				@foreach($pertanyaan_umums as $pertanyaan_umum)
                                        <tr>
                                            ...
                                            <td class="nowrap">{{$pertanyaan_umum->pertanyaan_pertanyaan_umums}}</td>
                                            <td class="nowrap">{!! nl2br($pertanyaan_umum->jawaban_pertanyaan_umums) !!}</td>
                                        </tr>
                                        @php($no++)
                                    @endforeach
                                @else
					...
					<br/>
				   	{{ $pertanyaan_umums->appends(Request::except('page'))->links('vendor.pagination.custom') }}
                </div>
            </div>
        </div>
    </div>
