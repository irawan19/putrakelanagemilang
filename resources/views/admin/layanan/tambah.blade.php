    ...
    <div class="row g-4 mb-4">
        <div class="col-md-12">
            <div class="card">
				...
				<form class="form-horizontal m-t-40" enctype="multipart/form-data" action="{{ URL('dashboard/layanan/prosestambahdetail') }}" method="POST">
					{{ csrf_field() }}
                    <div class="card-body">
						<div class="mb-3">
						...
						<div class="mb-3">
                            ...
                            <select class="form-select" aria-label="icon_layanan_details" name="icon_layanan_details">
				                @foreach($icon_layanan_details as $icon_layanan_detail)
				                    <option value="{{ $icon_layanan_detail['icon'].'_'.$icon_layanan_detail['nama'] }}" {{ Request::old('layanan_details') == $icon_layanan_detail['icon'].'_'.$icon_layanan_detail['nama'] ? $select='selected' : $select='' }}>{{ $icon_layanan_detail['nama'] }}</option>
				                @endforeach
                            </select>
                            {{\App\Helpers\General::pesanErrorForm($errors->first('icon_layanan_detail'))}}
                        </div>
                        <div class="mb-3">
                            <label class="form-label" for="judul_layanan_details">Judul <b style="color:red">*</b></label>
                            <input class="form-control {{ \App\Helpers\General::validForm($errors->first('judul_layanan_details')) }}" id="judul_layanan_details" type="text" name="judul_layanan_details" value="{{Request::old('judul_layanan_details')}}">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control {{ \App\Helpers\General::validForm($errors->first('konten_layanan_details')) }}" id="konten_layanan_details" name="konten_layanan_details" rows="5">{{Request::old('konten_layanan_details')}}</textarea>
